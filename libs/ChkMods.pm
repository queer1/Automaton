...
#foreach my $modexists (@exists) {
# if ("$modexists" != ''){
#}
#}
#eval {
#  require "$modexists.pm";
#  "$modexists.pm"->import();
#};
#&log_it("In $subname :: eval returned $@","debug","3","$caller");
#last;

#  if ("$netssh" eq "$mod") {
#  } elsif ("$netssh2" eq "$mod2") {
#    &log_it("In $subname :: using $mod2","debug","3","$caller");
#    eval {
#      require "$mod2.pm";
#      "$mod2.pm"->import();
#    };
#    &log_it("In $sub_name :: eval returned $@","debug","3","$caller");
#  } else {
#    &log_it("In $sub_name :: Module $mod or $mod2 is required without them this wont work","error","1","$caller");
#  }
