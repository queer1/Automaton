...
#
########  NB --- REMEMBER !!
# This Module is an absolute bitch to install
# Need to simplify this, I think its already part of Centos/Redhat..
##
# New Module ChkMods, checks to see if the modules are available, 
# if they arent this program will bomb out. 
##
...
sub ssh_rcon {
  ...
  my %un_key;
  #my $files = [ '/usr/home/host/.ssh/id_rsa' ];
  ...
  ## Determining the priority of logging to determin whether or not to set the verbose flag for the ssh connection
    ...
    case "Net::OpenSSH" {
      ...
      switch ($rcon_meth) {
        case  "rsa" {
          ...
          #$ssh = $ssh_mod->new("$rcon_host", debug=>"-1");
          #$ssh->debug(-1);
      ...
      $ssh->error and die "remote command failed: " . $ssh->error;
      &log_it("\n$host_output","debug","3","$caller");
      ##$ssh->scp_get("$autpwd/audit.img", "$mac_path/");
      $ssh->scp_get("$autpwd/audit.tar.gz", "$mac_path/");

      #my ($in_pipe, $out_pipe, undef, $pid) = $ssh->open_ex( { stdin_pipe => 1, stdout_pipe => 1, tty => "true" }, "sudo /bin/sh /opt/vFats/test-data-gather.sh" ) or die "open_ex failed: " . $ssh->error;

      #my ($spty, $spid) = $ssh->open2pty("sudo /bin/sh /opt/vFats/test-data-gather.sh") or die "remote command failed: " . $ssh->error;
      #print "$sin , $sout, $spid\n";
      #$ssh->system("sudo tar -cvvzf /opt/vFats/audit.tar.gz /opt/vFats/audit && dd if=/opt/vFats/audit of=/opt/vFats/audit.img") ;
      #$ssh->error and die "remote command failed: " . $ssh->error;
      ...
      &log_it("Attempting to ssh into server $rcon_host","debug","3","$caller");
      ##die (&log_it("This method is currently disabled because its an absolute pain to install the module","error","1","$caller"));
      #die("Sorry, Bye Now");
      switch ($rcon_meth) {
        case  "rsa" {
          ##
      ...
      #(push(@host_output), $err) = ssh->cmd("uname -n"); 
      #(push(@host_output), $err) = ssh->cmd("ls -al /"); 
      #(push(@host_output), $err) = ssh->cmd("df -h"); 
      #chomp $out;
      ...
      ######### CURRENTLY DISABLED, Net::SSH2 seems to be a bit buggy, threw up errors for no reason
      ## Left here to add and test much later if necessary.

      #my $ssh2 = Net::SSH2->new(trace => -1);
      #my $ssh2 = Net::SSH2->new();
      #$ssh2->trace(-1);
      #$ssh2->connect('10.19.95.22') or die $!;
      #$ssh2->connect('10.19.95.22') or die "Unable to connect Host $@ \n";
      #$ssh2->auth_list('host');
      #$ssh2->auth_publickey('automaton','/home/host/.ssh/id_rsa.pub','/home/host/.ssh/id_rsa') or die "WTF :: $!\n";

      ########## NET::SSH2 Experiments - the shit dont work 
      #my $rcon=N
      #my $chan = $ssh2->channel();
      #$chan->exec('ls -al .');
      #print "$chan";
      #$ssh2->auth_publickey('host',
      #'/home/host/.ssh/id_rsa.pub', #testing on localhost
      #'/home/host/.ssh/id_rsa',     
      #) or die "This has failed :: @! , $!\n";
      #my $chan = $ssh2->channel();
      #$chan->exec('ls -la');
      #while (<$chan>){ print } 
      #my $ssh = Net::SSH::Perl->new("$rcon_host");
      #   if ($ssh2->auth_keyboard('automaton')) {
      #   my $chan = $ssh2->channel();
      #   $chan->exec('ls -al .');
      #   print "$chan";
      #my $sftp = $ssh2->sftp();
...
# apacheCheck=`egrep -ir "RewriteEngine" /etc/apache*`
# procInfo=`ps auwx 2>&1|awk '{print $1"\t"$9"\t"$11 $12 $NF}'|uniq`
# pkgIns=`dpkg-query -W -f='${Package}\t${Version}\n' 2>&1`
# cpuInfo=`egrep -i "(CPU)|(cpu)" /var/log/dmesg 2>&1`
# memInfo=`egrep -i "(memory)|(Ethernet)" /var/log/dmesg 2>&1`
# modInfo=`lsmod 2>&1`
